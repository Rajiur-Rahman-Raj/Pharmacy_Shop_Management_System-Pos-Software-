            ...
            <div class="page-header">
                ...
                <div class="page-header-breadcrumb">
                    <ul class="breadcrumb-title">
                        ...
                        <li class="breadcrumb-item"><a href="{{url('/admin/return_list')}}">Return List</a>
                        </li>
                    </ul>
                </div>
            </div>
                                    ...
                                    <table id="invoiceTable" class="table table-striped table-bordered nowrap">
                                        ...
                                        @if(!empty($invoices))
                                            @foreach($invoices as $invoice)
                                                <tr>
                                                    <td>{{ $i++ }}</td>
                                                    <td><a href="{{url('/admin/view-return/'.$invoice->id)}}">{{ $invoice->id }}</a></td>
                                                    <?php
                                                        $customer_details = unserialize($invoice->customer_details);
                                                    ?>
                                                    <td>{{ $customer_details['customer_name'] }}</td>
                                                    <td>{{ date('d M Y', strtotime($invoice->created_at))}}</td>
                                                    <td>{{ $invoice->total - $invoice->return_charge }}</td>
                                                    <td>{{ $invoice->return_charge }}</td>
                                                    <td>
                                                        <a href="{{url('/admin/view-return/'.$invoice->id)}}" title="Invoice" class="btn btn-info waves-effect waves-light"><i class="fa fa-window-restore"></i></a>
                                                        <a href="{{url('/admin/receipt-return-invoice/'.$invoice->id)}}" title="Return Invoice" class="btn btn-primary waves-effect waves-light"><i class="fa fa-fax"></i></a>
                                                        <a href="javascript:" rel="{{$invoice->id}}" rel1="delete_return" class="btn btn-danger waves-effect waves-light deleteRecord" title="Delete"><i class="fa fa-trash"></i></a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        @endif
